...
alert(`Bem vindo ao caixa eletronico da GrowDev`);

alert(`Seu Saldo é de ${saldo} GrowCoins`);

let saque = Number(prompt(`Insira o valor a sacar, este deve ser positivo, multiplo de 5 e menor que o saldo:`));

if((saque+taxaSaque)>saldo){
    alert(`Valor inserido maior que o disponível em sua conta!`);
}else{
    novoSaldo = saldo-(taxaSaque+saque);
    alert(`Saque realizado com sucesso, seu saldo atual é de ${novoSaldo} GrowCoins.`)
}
